...
use App\Proveedores;
use App\Exports\VentasProveedorTotal;
...
class VentasProveedor implements WithMultipleSheets
{
	 private $marca;
        ...
        public function sheets(): array
    {
     if ($this->hojas==1){
        ...
        $this->ventageneral = DB::connection('retail')->table('VENTASDET')->SELECT(DB::raw('VENTASDET.COD_PROD AS COD_PROD'),
            ...
            DB::raw('PRODUCTOS_AUX.PROVEEDOR AS PROVEEDOR'),  
         ...
         $this->sheets[]=new VentasProveedorPorMes(1,"a<",1,$this->hojas,$this->ventageneral);
        $this->hojas=$this->hojas+1;


      } 

       $RESULTS= DB::connection('retail')->table('VENTASDET')->SELECT(DB::raw('PRODUCTOS_AUX.PROVEEDOR as CODIGO' ),
            DB::raw('PROVEEDORES.NOMBRE as DescriM' ),
            DB::raw('PRODUCTOS.LINEA As Linea' )

           
          )
         ->leftJoin('PRODUCTOS', 'PRODUCTOS.CODIGO', '=', 'VENTASDET.COD_PROD')
           ->leftJoin('PRODUCTOS_AUX', 'PRODUCTOS_AUX.CODIGO', '=', 'VENTASDET.COD_PROD')
        ...
        $this->sheets[]= new VentasProveedorTotal($this->ventageneral,$RESULTS);
         ...
         foreach ($RESULTS as $KEY  => $value) {
          ...
          $this->sheets[]= new VentasProveedorPorMes($value->CODIGO,$descrim,$value->Linea,$this->hojas,$this->ventageneral);
