...
class VentasProveedorPorMes implements FromArray, WithTitle,WithEvents,ShouldAutoSize,WithColumnFormatting
{
	private $CODIGO;
    ...
    private $descuentos; 
      public function __construct(int $CODIGO, string $DESCRIPCION,int $CODIGOL,int $hojas, array $ventageneral=[] )
    {
        $this->CODIGO = $CODIGO;   
     ...
     public function  array(): array
    {
      ...
      if ($this->hojas==2){
        ...
        foreach ($this->ventageneral as $key=> $value) {
            ...
            if($this->ventageneral[$key]->PROVEEDOR==$this->CODIGO  ){
