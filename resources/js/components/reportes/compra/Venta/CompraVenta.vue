				        ...
				        <table class="table" v-if="datosFilas !== null">
						  ...
						  <tbody>
						    <tr style="font-size: 12px" v-for="(compras, index) in filterProductos(responseCompras, datosFilas,datosProveedor)">
						      ...
						      <td>{{new Intl.NumberFormat("de-DE", {style: "decimal", decimal: "0"}).format(compras.PRECIO_UNIT)}}</td>
						      <td>{{new Intl.NumberFormat("de-DE", {style: "decimal", decimal: "0"}).format(compras.TOTAL)}}</td>
						      <td>{{new Intl.NumberFormat("de-DE", {style: "decimal", decimal: "0"}).format(compras.DESCUENTO)}}</td>
						    </tr>
						  </tbody>
						</table>
				      </div>
						...
						<table class="table table-sm">
						  ...
						  <tbody>
						    <tr v-for="(proveedores, index) in filterItems(responseSeccion, seccion.SECCIONES)">
						      ...
						      <td>{{new Intl.NumberFormat("de-DE", {style: "decimal", decimal: "0"}).format(proveedores.DESCUENTO)}}</td>
	       ...
	       habilitar_insert() {
	        ...
	        generarConsulta(){
	         ...
	         llamarDatos(){
	        	...
	        	if(this.generarConsulta() === true) {
	        		me.cargado = true;
	        	Common.generarReporteCompraVentaSeccionCommon(this.datos).then(data => {
