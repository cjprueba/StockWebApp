	...
	<div class="container-fluid mt-4">
	    ...
	    <div class=" ml-2 row my-2">
				<div class="custom-control custom-switch">
					<input type="checkbox" class="mr-4 custom-control-input" id="switchMayorista" v-model="checkedMayorista">
					<label class="custom-control-label" for="switchMayorista">Precio Mayorista</label>
				</div>
		</div>
    		...
    		<div class="card">
      			<div class="card-body">
      				...
      				<!-- ----------------------------------------- CODIGO ------------------------------------------------------------- -->
						...
						<div class="col-3">
		    					<label class="mr-1">Código De Nota</label>
		    					<remision-filtrar ref="componente_textbox_remision" :codigo="codigo_remision" @codigo="mostrarRemision" @cliente="cargarCodigo" @moneda="cargarMoneda" @usuario="enviar_nombre"></remision-filtrar>
						</div>

						<!-- ------------------------------------------ USUARIO ------------------------------------------------- -->
		
						<div class="col-sm">
			               	<usuario-nombre @id='enviar_id' @nombre_usuario='enviar_nombre' :nombre='nombreUsuario' v-bind:class="{ 'is-invalid': validar.nombreUsuario }"></usuario-nombre>
			            </div> 
				...
				<div class="row mt-2">
			    	...
			    	<div class="col">
							<label>Línea del Producto</label>
							<input v-bind:class="{ 'is-invalid': validar.linea }" v-model="producto.linea" type="text" class="form-control form-control-sm" disabled>
					</div>
				</div>
				...
				<div class="row mt-3">
					...
					<div class="col-3" v-if="checkedMayorista">
					...
					<div class="col">
					    <label for="inlineFormInputGroupUsername">Descuento</label>
					    <div class="input-group input-group-sm"  v-bind:class="{ 'is-invalid': validar.descuento }">
						    <input type="text" class="form-control form-control-sm" id="inlineFormInputGroupUsername" v-model="producto.descuento" disabled>
    		...
    		<div class="card">
    			...
    			<!-- -------------------------------- BOTONES LIMPIAR, GUARDAR, MODIFICAR Y ELIMINAR --------------------------------- -->
      			...
      			<div class="card-body">
      				...
      				<div class="row-1"> <div class="col text-center">
      					<button v-on:click="limpiar()" type="button" class="btn btn-outline-primary btn-block">Nuevo</button>
      				</div></div>
					
					<div v-if="btnguardar" class="row mt-4"> <div class="col text-center">
						<button v-on:click="guardar" type="button" class="btn btn-outline-success btn-block">Guardar</button>
					</div></div>
					
					<div v-else class="row mt-4"> <div class="col text-center">
						<button v-on:click="modificarRemision" type="button" class="btn btn-outline-warning btn-block">Modificar</button>
					</div></div>

					<div class="row mt-4"> <div class="col text-center">
						<button v-on:click="eliminar()" type="button" class="btn btn-outline-danger btn-block">Eliminar</button>
					</div></div>
					
					<div class="row mt-4"> <div class="col text-center">
						<button v-on:click="remisionPDF()" type="button" class="btn btn-outline-info btn-block" :disabled="!!btnguardar">Imprimir</button>
					</div></div>
				</div>
      ...
      props: ['candec', 'monedaCodigo', 'tab_unica', 'id_sucursal'],
		...
		methods:{
			...
			agregarProductoRemision(){
	            ...
	            if (me.producto.codigoProd.length === 0) {
	                me.validar.codigoProd = true;
	                return;
	            } else {
	                me.validar.codigoProd = false;
	            ...
	            if (me.producto.precioUnitario.length === 0  || me.producto.precioUnitario === '0') {
	            ...
	            if (me.producto.descuento.length === 0) {
	            ...
	            me.agregarFilaTabla(me.producto.codigoProd, me.producto.descripcion, me.producto.cantidad, precioConDescuento, iva, precio, me.ivaProducto, me.producto.cantidadExistente, me.producto.descuento);
	        ...
	        agregarFilaTabla(codigo, descripcion, cantidad, precio, iva, total, iva_porcentaje, stock, descuento){
	            ...
	            productoExistente = Common.existeProductoRemisionDataTableCommon(tableProductos, codigo, 2);
	            ...
	            if (productoExistente.respuesta == true) {
	            	...
	            	me.editarCantidadProducto(tableProductos, cantidadNueva, productoExistente.precio, iva_porcentaje, productoExistente.stock, productoExistente.row, productoExistente.descuento);
	            	return;
			...
			editarCantidadProducto(tabla, cantidad, precio_producto, iva_producto, stock, row, descuento){
			 ...
			 // INICIAR EL DATATABLE PRODUCTOS 
                ...
                var tableProductos = $('#tablaProductos').DataTable({
				        ...
				        "buttons": [
				            ...
				            { extend: 'print', text: '<i class="fa fa-print"></i>', titleAttr: 'Imprimir', className: 'btn btn-secondary', title: 'NotaDeRemision', messageTop: 'Productos registrados para Nota de Remision' }
				        ],
				        "columnDefs": [
				            {
				                "targets": [ 8 ],
                ...
                $('#tablaProductos').on('click', 'tbody tr', function() {
                    ...
                    me.editarIvaProducto = tableProductos.row(this).data().IVA_PORCENTAJE;
                    me.editarStock = tableProductos.row(this).data().STOCK;
                    me.editarDescuento = tableProductos.row(this).data().DESCUENTO;
                ...
                $('#editarFila').on('click', function() {
                	...
                	me.editarCantidadProducto(tableProductos, me.editarCantidad, me.editarPrecio, me.editarIvaProducto, me.editarStock, me.editarRow, me.editarDescuento);
