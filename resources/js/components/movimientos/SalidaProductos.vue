<template>
	<div v-if="$can('movimientos.salidadeproducto') && $can('movimientos')" class="container-fluid mt-3">
		<div class="row">
            ...
            <div class="col-md-12 mt-3">
                <div class="section-title">
                    ...
                    <p>La salida de productos pueden realizarce por distintos motivos.</p>
                </div>
            </div>
					...
					<div class="card border-bottom-info mb-3">
					  <div class="card-body">
					    ...
					    <div class="row">
                            ...
                            <div class="col-md-3">
                                <label>Tipo</label>
                                <select v-model="devolucion.TIPO" class="custom-select custom-select-sm" v-bind:class="{ 'is-invalid': validar.TIPO }">
                                        <option :value="null">SELECCIONAR</option>
                                        <option :value="1">AVERIÓ</option>
                                        <option :value="2">VENCIDO</option>
                                        <option :value="3">ROBADO</option>
					    	...
					    	<div class="col-md-3">
					    		<lote-general v-model="producto.LOTE" :value="producto.LOTE" @lote_producto="lote_producto" ref="componente_lote"></lote-general>
					    	</div>
					    	...
					    	<div class="col-md-12 mt-3">
					    		<table id="tablaSalida" class="display nowrap table table-striped table-bordered table-sm mb-3" style="width:100%" >
					                <thead>
					                    <tr>
					                        <th>#</th>
					                        <th>Codigo</th>
    ...
    export default {
      ...
      methods: {
      	cargarLotes(codigo){
      		...
      		me.$refs.componente_lote.obtenerDatosLote(codigo, me.devolucion.MONEDA);
        ...
        agregarProducto(){
            ...
            let me = this;
            var tableSalida = $('#tablaSalida').DataTable();
            ...
            productoExistente = Common.existeProductoLoteDataTableCommon(tableSalida, me.producto.CODIGO, me.producto.LOTE, 3);
            ...
            if (productoExistente.respuesta == true) {
            	...
            	if (cantidadNueva <= me.producto.STOCK) {
            		me.editarCantidadProducto(tableSalida, cantidadNueva, productoExistente.costo, productoExistente.row);
            	} else {
            ...
            if (tableSalida.rows().data().length > 0) {
            ...
            tableSalida.on( 'order.dt search.dt', function () {
                tableSalida.column(0, {search:'applied', order:'applied'}).nodes().each( function (cell, i) {
                    cell.innerHTML = i+1;
                } );
            } ).draw();
            ...
            tableSalida.columns.adjust().draw();
        ...
        controlador() {
            ...
            if (me.devolucion.TIPO === null) {
                me.validar.TIPO = true;
                falta = true;
            } else {
        ...
        guardar() {
        	...
        	// ------------------------------------------------------------------------
      		...
      		Swal.fire({
				title: 'Estas seguro ?',
				text: "Dar salida estos productos ? !",
				type: 'warning',
				...
				preConfirm: () => {
				    return Common.guardarSalidaProductoCommon(data).then(data => {
        ...
        mounted() {
          ...
          tableSalida.on('click', 'tbody tr #eliminarProducto', function() {
            ...
            Swal.fire({
				title: 'Estas seguro ?',
				text: "Eliminar el producto " + tableSalida.row($(this).parents('tr')).data().CODIGO + " !",
				type: 'warning',
				showLoaderOnConfirm: true,
				showCancelButton: true,
				confirmButtonColor: '#d33',
				...
				if (result.value) {
	                ...
	                tableSalida.row($(this).parents('tr')).remove().draw(); 
            ...
            tableSalida.on('click', 'tbody tr a#cod_prod_tabla', function() {
